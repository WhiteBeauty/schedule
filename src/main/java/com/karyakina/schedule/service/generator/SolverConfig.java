...
public record SolverConfig(
    ...
    public record Weights(
        ...
        public static Weights defaults() {
            ...
            //
            // dayImbalance пробовала поднять 6->35 в расчёте на то, что это остановит
            // "убрать окна за счёт пустых дней" — эмпирически стало ЗАМЕТНО ХУЖЕ (17->34
            // окна у групп): видимо, сильный dayImbalance начал расталкивать пары по всем
            // дням настолько, что внутри самих дней появлялись разрывы (цель "есть пара в
            // каждый день" и цель "пары внутри дня подряд без дырок" тянут в разные
            // стороны сильнее, чем я предполагала). Возвращаю dayImbalance обратно к 6 —
            // это было лучшее эмпирически подтверждённое сочетание из всех, что пробовала.
