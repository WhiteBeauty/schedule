...
public record SolverConfig(
    ...
    public record Weights(
        ...
        public static Weights defaults() {
            ...
            //
            // НО: это создало новый перекос — dayImbalance=6 был слишком слаб рядом с
            // groupGap=55, и у алгоритма появился дешёвый способ "убрать окна": набить
            // пары плотно в 2-3 дня без единого разрыва, а остальные дни оставить почти
            // пустыми (1 пара или вообще ничего) — формально "окон" меньше, а по факту
            // расписание стало ещё более рваным по дням. Подняла dayImbalance 6->35 —
            // ближе по порядку величины к groupGap, чтобы "нет окон за счёт пустых дней"
            // перестало быть выгодным решением для локального поиска.
            return new Weights(55, 22, 60, 35, 20, 8, 1.5, 2);
        }
    }
